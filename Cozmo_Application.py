...
class Battery(cozmo.annotate.Annotator):
    def apply(self, image, scale):
        ...
        text = cozmo.annotate.ImageText('BATT %.1fv \nCharging : %r' % (batt, self.world.robot.is_charging), color='green')
        text.render(d, bounds)

class ThreadSecurite(Thread):
    def __init__(self,mcp):
        Thread.__init__(self)
        self.mcp = mcp

    def run(self):
        while (not self.mcp.interrupted):
            securite = False
            if (self.mcp.robot.is_falling or self.mcp.robot.is_picked_up) :
                securite = True
            if securite :
                self.mcp.robot.abort_all_actions(True)
            self.mcp.securite = securite
            time.sleep(1)
...
class MCP():
    ...
    def gestion_batterie(self):
        ...
        if (self.robot.battery_voltage < self.threshold_min and not self.robot.is_charging):
            self.ActionPrioritaire = "CHARGER" 
        if (self.robot.battery_voltage > self.threshold_max and self.robot.is_charging) :
            self.ListeNormale.append("SORTIR_CHARGEUR")

    def find_charger(self):
        while(True):
            for i in range(0, int(math.ceil(360/self.robot.camera.config.fov_x.degrees))) :
                self.robot.turn_in_place(self.robot.camera.config.fov_x).wait_for_completed()
                try: 
                    seen_charger = self.robot.world.wait_for_observed_charger(timeout=2,include_existing=True)
                    break
                except:
                    seen_charger = None
    ...
    def go_to_charger(self):
        ...
        if (charger is None):
            return charger
        action = self.robot.go_to_object(charger,distance_from_object=distance_mm(40), in_parallel=False, num_retries=5)
    ...
    def gestion_actions(self):
        ...
        if (self.securite):
            time.sleep(5)
            self.ActionPrioritaire = ""
            self.ListeNormale = []
            return

        if (not self.ActionPrioritaire):
            self.ListeNormale = []
            action = self.ActionPrioritaire
            self.ActionPrioritaire = ""
        ...
        if (action == "CHARGER"):
            ...
            self.robot.set_head_angle(cozmo.robot.MAX_HEAD_ANGLE).wait_for_completed()
            self.robot.backup_onto_charger(10)
            if (self.robot.is_charging):
                self.sleepy();
        elif (action == "VISAGE"):
            ...
            if (cube is not None):
                ...
                #self.robot.set_lift_height(0).wait_for_completed()
                #time.sleep(1)
                self.robot.pickup_object(cube,num_retries=3)
                time.sleep(1)
                self.robot.place_object_on_ground_here(cube)
                self.robot.drive_straight(distance_mm(-60), speed_mmps(10)).wait_for_completed()
                self.robot.set_head_angle(cozmo.robot.MAX_HEAD_ANGLE).wait_for_completed()
    ...
    def __init__(self):
        ...
        ###CONFIGURATION#####
        self.temps_oubli_inconnus = 60 #temps (s) avant qu'il redise bonjour à un inconnu
        self.delai_occupation = 10 #temps (s) entre deux activités de Cozmo
        self.timide = False
        self.be_quiet = True
        self.master="clement"
...
cozmo.run_program(mcp.Manage,use_viewer=True,force_viewer_on_top=False)
